...
import { generalData, audioData, cubeTextureData, mazeData, playerData, ambientLightData, directionalLightData, spotLightData, flashLightData, shadowsData, fogData, collisionDetectionData, cameraData, doorData } from "./default_data.js";
...
export default class ThumbRaiser {
    constructor(generalParameters, audioParameters, cubeTexturesParameters, mazeParameters, playerParameters, ambientLightParameters, directionalLightParameters, spotLightParameters, flashLightParameters, shadowsParameters, fogParameters, collisionDetectionParameters, fixedViewCameraParameters, firstPersonViewCameraParameters, thirdPersonViewCameraParameters, topViewCameraParameters, miniMapCameraParameters, buildingParameters, tasksParameters) {
        ...
        this.buildingParameters = merge({}, buildingParameters);
        this.tasksParameters = merge([], tasksParameters);
        ...
        if (this.tasksParameters != undefined) {
          for (let i = 0; i < this.tasksParameters.length; i++) {
            console.log(this.tasksParameters[i]);
            this.tasks.push(this.tasksParameters[i]);
          }
        }
        ...
        this.buildPopupPanel();

        // Build the automatic navigation panel
        this.buildAutomaticNavigationPanel();


        // Creates the variable repsonsible for knowing the previous when going through a passage
        this.oldFloor = "";

        // Creates the booleans for floor change
        this.floorChangePassage = false;
        this.floorChangeElevator = false;

        // Creates the variable responsible for automatic navigation
        this.automaticNavigation = false;

        //Creates the variable responsible for tween handling
    ...
    buildPopupPanel() {
      ...
      this.popUpContent.addEventListener('change',() => {
        console.log("Selected floor: " + this.popUpContent.value);
      });

      const okButtonPopUp = document.getElementById("popup-ok");
      okButtonPopUp.addEventListener("click", event => {
        this.mazeParameters.url = this.popUpContent.value;
        this.popUp.style.display = "none";
        this.changeFloor();
      });

      this.floorChangeMessage = document.getElementById("floor-change-message");
      this.floorChangeMessageContent = document.getElementById("floor-change-message-content");

      const okButtonFloorChange = document.getElementById("floor-change-message-ok");

      okButtonFloorChange.addEventListener("click", event => {
        this.floorChangeMessage.style.display = "none";
      });

    }

    buildAutomaticNavigationPanel() {
      this.pendingTasksSelect = document.getElementById("pending-tasks");

      if (this.tasks.length > 0) {
        for (let i = 0; i < this.tasks.length; i++) {
          const option = document.createElement("option");
          option.text = this.tasks[i].name;
          option.value = i;
          this.pendingTasksSelect.add(option);
        }
      }

      this.pendingTasksSelect.addEventListener('change',() => {
        console.log("Selected task: " + this.pendingTasksSelect.value);
      });

      this.automaticNavigationMessage = document.getElementById("automatic-navigation-in-progress");

      const beginAutomaticNavigationButton = document.getElementById("automatic-navigation-begin");

      beginAutomaticNavigationButton.addEventListener("click", event => {

        this.taskIndex = this.pendingTasksSelect.value;
        if (!this.tasks[this.taskIndex].name.includes("(done)")) {
          this.automaticNavigationMessage.style.display = "block";
          if (this.mazeParameters.url !== this.tasks[this.taskIndex].floors[0]) {     // if it is a different beginning floor, change the floor to that one
            this.mazeParameters.url = this.tasks[this.taskIndex].floors[0];
            this.changeFloor();
          } else {
            this.tasks[this.taskIndex].floors.shift();      // if it is the same beginning floor, just remove it from the array
          }

          const coordinateCartesian = this.maze.cellToCartesian(this.tasks[this.taskIndex].path[0][0]);

          if (this.player.position.x !== coordinateCartesian.x || this.player.position.z !== coordinateCartesian.z) {     // if it is a different beginning position, change the position to that one
            this.player.position.set(coordinateCartesian.x, this.player.position.y, coordinateCartesian.z);
          }
        ...
        if (event.target.id == "canvas") {
            ...
            if (event.buttons == 0 || event.buttons == 1 || event.buttons == 2) {
                ...
                if (event.buttons == 0) { // No button down
                    ...
                    // Check if the mouse is hovering over an object using raycasting
                  const pointer = new THREE.Vector2();
                  pointer.x = (event.clientX / window.innerWidth) * 2 - 1;
                  pointer.y = -(event.clientY / window.innerHeight) * 2 + 1;

                    this.checkHoverTarget(this.mouse, pointer);
                }
  ...
  checkHoverTarget(mouse, pointer) {

    // Gets all the cameras that are currently visible
    const cameras = (this.miniMapCamera.checkBox.checked ? [this.miniMapCamera] : []).concat(this.visibleViewportCameras);

    switch (this.activeViewCamera.view) {
      case "mini-map":
        this.maze.hoverDoor(this.raycaster, pointer, this.miniMapCamera);
        this.maze.hoverElevator(this.raycaster, pointer, this.miniMapCamera);
        break;
      case "top":
        this.maze.hoverDoor(this.raycaster, pointer, this.topViewCamera);
        this.maze.hoverElevator(this.raycaster, pointer, this.topViewCamera);
        break;
      case "first-person":
        this.maze.hoverDoor(this.raycaster, pointer, this.firstPersonViewCamera);
        this.maze.hoverElevator(this.raycaster, pointer, this.firstPersonViewCamera);
        break;
      case "fixed":
        this.maze.hoverDoor(this.raycaster, pointer, this.fixedViewCamera);
        this.maze.hoverElevator(this.raycaster,pointer, this.fixedViewCamera);
        break;
      case "third-person":
        this.maze.hoverDoor(this.raycaster, pointer, this.thirdPersonViewCamera);
        this.maze.hoverElevator(this.raycaster, pointer, this.thirdPersonViewCamera);
        break;
    }
  }
    /*
    ...
    update() {
        if (!this.gameRunning) {
            if (this.audio.loaded() && this.maze.loaded && this.player.loaded) { // If all resources have been loaded
              if (this.floorChangePassage) {
                this.maze.setPositionToPassageExit(this.oldFloor);
                this.floorChangePassage = false;
              }
                ...
                types.forEach(type => {
                    type.forEach(clip => {
                        ...
                        else if (clip.position == "exit") { // Positional audio object (maze passage location)
                            this.scene.add(clip.source);
                            if (this.maze.passageLocations.length > 0) {
                              clip.source.position.set(this.maze.passageLocations[0].x, this.maze.passageLocations[0].y, this.maze.passageLocations[0].z);
                            }
                        }
            ...
            if (!this.animations.actionInProgress) {
                // Check if the player found a passage
                const passageIndex = this.maze.foundPassage(this.player.position);
                if (passageIndex !== undefined && !this.automaticNavigation) {
                  // Update maze parameters with destination URL
                  this.userInterface.close();

                  // Reset the player model
                  this.animations.fadeToAction("Idle", 0.2);

                  this.changeFloorPassage(passageIndex);
                } else if (this.maze.foundElevator(this.player.position) && !this.automaticNavigation) {
                    // Checks if the player found an elevator

                    this.userInterface.close();
                  this.animations.fadeToAction("Idle", 0.2);

                  this.changeFloorElevator();
                    ...
                    if (this.maze.collision(this.collisionDetectionParameters.method, position, this.collisionDetectionParameters.method != "obb-aabb" ? this.player.radius : this.player.halfSize, directionRad - this.player.defaultDirection)) {
                        ...
                        if (this.automaticNavigation && !this.tweenInProgress) {
                          console.log("path length: " + this.tasks[this.taskIndex].path[0].length + " floors length: " + this.tasks[this.taskIndex].floors.length + "")
                          if (this.tasks[this.taskIndex].path[0].length > 0) {
                            this.tweenInProgress = true;
                            this.taskExecution(this.tasks[this.taskIndex].path[0].shift());

                          } else if (this.tasks[this.taskIndex].floors.length > 0) {
                            this.tasks[this.taskIndex].path.shift();
                            this.mazeParameters.url = this.tasks[this.taskIndex].floors.shift();
                            this.changeFloor();

                          } else {
                            this.automaticNavigationMessage.style.display = "none";
                            this.tasks[this.taskIndex].name = this.tasks[this.taskIndex].name + " (done)";
                            this.pendingTasksSelect.options[this.taskIndex].text = this.tasks[this.taskIndex].name;
                            //this.pendingTasksSelect.removeChild(this.pendingTasksSelect.firstChild);
                            this.tweenInProgress = false;
                            this.automaticNavigation = false;
    ...
    changeFloor(){
      console.log("changing floor to " + this.mazeParameters.url);
      this.userInterface.close();
      this.gameRunning = false;
      this.floorChangeMessageContent.querySelector('label').textContent = "Changing floor to " + this.urlToFloorName(this.mazeParameters.url) + " of " + this.urlToBuildingName(this.mazeParameters.url);
      this.floorChangeMessage.style.display = "block";

      if (this.collisionDetectionParameters.boundingVolumes.visible) {
        this.setBoundingVolumesVisibility(false);
      }
      this.scene.remove(this.maze);
      this.maze = new Maze(merge({}, mazeData, this.mazeParameters));
      console.log(this.maze);
      console.log(this.maze.initialPosition);
      this.scene.add(this.maze);
      console.log(this.player.position);
      this.player.direction = this.maze.initialDirection;

      console.log("changed floor to " + this.mazeParameters.url);
    }

  changeFloorPassage(passageIndex){
    this.oldFloor = this.mazeParameters.url;
    console.log("old floor: " + this.oldFloor);
    this.mazeParameters.url = this.maze.passageDestinations[passageIndex];
    this.changeFloor();

    this.floorChangePassage = true;
    console.log("changed floor to " + this.mazeParameters.url);
  }

  changeFloorElevator() {
    if (this.popUp.style.display !== "block" && this.popUpContent.options.length > 0 && !this.automaticNavigation) {
      this.popUp.style.display = "block";

      if (this.popUpContent.options.length > 0) {
        this.popUpContent.options.length = 0;
      }
      for (let i = 0; i < this.maze.elevatorFloors.length; i++) {
        //popUpContent.options.add(new Option(this.urlToName(this.maze.elevatorFloors[i]), this.maze.elevatorFloors[i]));
        this.popUpContent.innerHTML += "<option value='" + this.maze.elevatorFloors[i] + "'>" + this.urlToFloorName(this.maze.elevatorFloors[i]) + "</option>";
      }
    }
      this.maze.openElevatorDoors();


  }

  taskExecution(coordinate) {
      console.log(coordinate);
      let oldCoordinate = this.maze.cartesianToCell(this.player.position);
      const coordinatePosition = this.maze.cellToCartesian(coordinate);
      const oldOrientation = this.player.direction;
      ...
      console.log("new coordinate: " + coordinate);
        ...
        // Rotate the robot to face the direction of the next coordinate
        if (coordinate[0] == oldCoordinate[0] - 1 && coordinate[1] == oldCoordinate[1] - 1){
          orientationValue =  5*(Math.PI / 4);

        } else if (coordinate[0] == oldCoordinate[0] - 1 && coordinate[1] == oldCoordinate[1] + 1){
          orientationValue = 3*(Math.PI / 4)

        } else if (coordinate[0] == oldCoordinate[0] + 1 && coordinate[1] == oldCoordinate[1] - 1){
          orientationValue = 7*(Math.PI / 4);

        } else if (coordinate[0] == oldCoordinate[0] + 1 && coordinate[1] == oldCoordinate[1] + 1){
          orientationValue = (Math.PI/4);

        } else if (coordinate[0] == oldCoordinate[0] + 1 && coordinate[1] == oldCoordinate[1]){
          orientationValue = 0;

        } else if (coordinate[0] == oldCoordinate[0] - 1 && coordinate[1] == oldCoordinate[1]){
          orientationValue = (Math.PI);

        } else if (coordinate[0] == oldCoordinate[0] && coordinate[1] == oldCoordinate[1] + 1){
          orientationValue = (Math.PI/2);

        } else if (coordinate[0] == oldCoordinate[0] && coordinate[1] == oldCoordinate[1] - 1){
          orientationValue = 3*(Math.PI/2);
        }

        // Set the duration of the tween (makes the animation flow better when the robot is not going to turn)

        let duration = 500;
        if (orientationValue == oldOrientation){
          duration = 0;
        }

    // Initialize tween to be chained
        let orientationTween = new TWEEN.Tween({
          rotationY : this.player.direction
        }).to({
          rotationY: THREE.MathUtils.radToDeg(orientationValue)
        }, duration).onUpdate(object => {
          this.player.direction = object.rotationY;
        }).dynamic(true);


    // Move the robot to the next coordinate
        orientationTween = orientationTween.chain(new TWEEN.Tween({
          positionX: this.player.position.x,
          positionZ: this.player.position.z
        })
          .to({
            positionX: coordinatePosition.x,
            positionZ: coordinatePosition.z
          }, 1000)
          .onUpdate(object => {
            this.animations.fadeToAction(this.player.shiftKey ? "Running" : "Walking", 0.2);
            this.player.position.set(object.positionX, this.player.position.y, object.positionZ);
        }).onComplete(() => {
          this.tweenInProgress = false;
        }).dynamic(true)).start();
  ...
  urlToFloorName(url){
      // Changes url string to the name of the floor
      let name = url.slice(16,23);
      name = name.charAt(0).toUpperCase() + name.slice(1);
      return name.slice(0, 5) + " " + name.charAt(5);
  }

  urlToBuildingName(url){
      // Changes url string to the name of the building
      let name = url.slice(23,32);
      name = name.charAt(0).toUpperCase() + name.slice(1);
      name = name.slice(0, 8) + " " + name.charAt(8);
      return name;
  }
  ...
  destroy() {
    ...
    // Dispose frame
    if (this.frame) {
      for (let i = this.frame.children.length - 1; i >= 0; i--) {
        const object = this.frame.children[i];
        if (object.geometry) object.geometry.dispose();
        if (object.material) object.material.dispose();
      }
      this.frame = null;
