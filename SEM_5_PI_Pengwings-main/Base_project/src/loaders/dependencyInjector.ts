...
export default ({ mongoConnection, schemas, controllers, repos, services,auth}: {
                    mongoConnection;
                    schemas: { name: string; schema: any }[],
                    controllers: {name: string; path: string }[],
                    repos: {name: string; path: string }[],
                    services: {name: string; path: string }[],
                    auth: {name: string; path:string }[] }) => {
  try {
    ...
    auth.forEach(m => {
      let authClass = require(m.path).default;
      let authInstance = Container.get(authClass);
      Container.set(m.name, authInstance);
    });
